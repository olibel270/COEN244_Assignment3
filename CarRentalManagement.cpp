...
CarRentalManagement::CarRentalManagement()				//Default Constructor
{
}
...
void CarRentalManagement::addCar(Car carToAdd)			//Function to Add a Car to inventory
{
...
//--------------------Rental Management------------------
void CarRentalManagement::rentCar(int customerID, string carType)
{	//When a customer desires a certain car type, one is assigned to him
...
void CarRentalManagement::returnCar(int customerID)
{
	...
	for (unsigned i = 0; i<customerList.size(); i++)		//Select Right customer from customerList
		if((int)customerList.at(i)->getId()==customerID)
		{
			...
			{											//If More than one car, define car to return
				cout<<"More than one car rented. Which car should be returned?"<<endl;
				cout<<setw(6)<<"Car ID"<<setw(15)<<"Car Type"<<endl;
				for (unsigned j=0; j<customerList.at(i)->carRented.size(); j++)
				{	//For each car rented by customer
